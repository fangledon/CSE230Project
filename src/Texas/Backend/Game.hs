...
-- >>> newGame 4 100 2 999
-- G {players = [P {hand = [<♣Q>,<♠5>], money = 96, wager = 4, isFolded = False, isAllIn = False, egComb = High []},P {hand = [<♦2>,<♠A>], money = 100, wager = 0, isFolded = False, isAllIn = False, egComb = High []},P {hand = [<♠9>,<♥8>], money = 100, wager = 0, isFolded = False, isAllIn = False, egComb = High []},P {hand = [<♦5>,<♦A>], money = 98, wager = 2, isFolded = False, isAllIn = False, egComb = High []}], public = [<♦3>,<♣7>,<♠6>,<♦Q>,<♦K>,<♠4>,<♠3>,<♠Q>,<♦J>,<♠2>,<♣6>,<♦8>,<♠K>,<♣8>,<♥10>,<♦6>,<♦7>,<♥Q>,<♣A>,<♣J>,<♦4>,<♣10>,<♥K>,<♥A>,<♣3>,<♣9>,<♥2>,<♥5>,<♣K>,<♠J>,<♥7>,<♥6>,<♥4>,<♦9>,<♠8>,<♥9>,<♣2>,<♠7>,<♥3>,<♠10>,<♣5>,<♣4>,<♥J>,<♦10>], phase = Preflop, ins = Ongoing, smallBlind = 2, dealerPos = 2, currentPos = 1, rng = StdGen {unStdGen = SMGen 13699080363776932830 4790414380670817565}}
    ...
    where dhi :: Int -> Game -> Game
          dhi p g | p == dealerPos g = dealcur p g
                ...
                where dealcur :: Int -> Game -> Game
                      dealcur pos (G pl pu p3 p4 p5 p6 p7 p8) = let (x, p:ys) = splitAt pos pl
                                                                    (f2, rest) = splitAt 2 pu in
                                                                        G (x ++ updHand p f2 : ys) rest p3 p4 p5 p6 p7 p8

actPlaceBet :: Int -> Game -> Game
actPlaceBet b g@(G pl p2 p3 p4 p5 p6 cp p8) = let (x,p:ys) = splitAt cp pl in
                                            G (x ++ placeBet p b : ys) p2 p3 p4 p5 p6 (npp g cp) p8
...
canContinue (G pl _ _ _ sb _ _ _) = all (\p -> money p >= 2 * sb) pl
